...
@implementation Scene{
    ...
    SKAction* pipeSoundAction;
    SKAction* medalSoundAction;
    SKAction* deadSoundAction;
...
- (id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        pipeSoundAction = [SKAction playSoundFileNamed :@"Pickup_Coin22.caf" waitForCompletion:NO];
        medalSoundAction = [SKAction playSoundFileNamed :@"Powerup15.caf" waitForCompletion:NO];
        deadSoundAction = [SKAction playSoundFileNamed :@"Explosion8.caf" waitForCompletion:NO];
        [self startGame];
    }
    return self;
}
...
{
    back = [SKScrollingNode scrollingNodeWithImageNamed:@"pixel nanigans rug" inContainerWidth:WIDTH(self)];
...
- (void)createFloor
{
    floor = [SKScrollingNode scrollingNodeWithImageNamed:@"building wall" inContainerWidth:WIDTH(self)];
...
- (void) createObstacles
{
    ...
    for(int i=0;i<nbObstacles;i++){
        ...
        SKSpriteNode * topPipe = [SKSpriteNode spriteNodeWithImageNamed:@"desks"];
        [topPipe setAnchorPoint:CGPointZero];
        [self addChild:topPipe];
        [topPipes addObject:topPipe];
        
        SKSpriteNode * bottomPipe = [SKSpriteNode spriteNodeWithImageNamed:@"desks"];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (wasted) {
        ...
        [bird bounce];
        //[[Tracking sharedInstance] trackUserEvent: @"tap" Value: @""];
    }
}
...
- (void) updateScore:(NSTimeInterval) currentTime {
    for(int i = 0; i < nbObstacles; i++) {
        ...
        if (X(topPipe) + WIDTH(topPipe) / 2 > bird.position.x && X(topPipe) + WIDTH(topPipe) / 2 < bird.position.x + FLOOR_SCROLLING_SPEED) {
            self.score += 1;
            ...
            if (self.score == 10 || self.score == 20 || self.score == 30 || self.score == 40) {
                [self runAction: medalSoundAction];
            } else {
                [self runAction: pipeSoundAction];
            }
        }
    }
}
...
- (void)didBeginContact:(SKPhysicsContact *)contact {
    ...
    [self runAction: deadSoundAction];
